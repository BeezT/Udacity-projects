    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            func displayError(_error:String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel!.text="Could not get token"
                }
            }
            ...
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode>=200 && statusCode<=299 else {
                displayError(_error: "status code is not 2XX")
                return
            }
            
            
            guard let data = data else {
                displayError(_error: "no data returned by the request")
                return
            }
            ...
            do {
                ...
                displayError(_error: "could not parse \(data) as JSON")
                return
            }
            
            guard let requestToken=parsedResult[Constants.TMDBResponseKeys.RequestToken] as? String else {
                displayError(_error: "no request token in \(parsedResult)")
                return
            }
            ...
            self.debugTextLabel!.text=requestToken
    ...
    private func loginWithToken(_ requestToken: String) {
        ...
        let task=appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            func displayError(_error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel!.text="could not login"
                }
            }
            ...
            guard let success=parsedResult[Constants.TMDBResponseKeys.Success] as? Bool, success == true else {
                displayError(_error: "Can not find \(Constants.TMDBResponseKeys.Success) in \(parsedResult)")
                return
            }
            print("logged in")
      ...
      let request=URLRequest(url: appDelegate.tmdbURLFromParameters(methodParameters as [String : AnyObject], withPathExtension: "/account"))
        ...
        let task=appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            do {
                ...
                displayError("can't parse the data as JSON \(data)")
                return
            }
            /* GUARD: Did TheMovieDB return an error? */
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
